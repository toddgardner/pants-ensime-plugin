...
_TEMPLATE = os.path.join('ensime', 'ensime')
...
class EnsimeGen(IdeGen):
  ...
  def register_options(cls, register):
    ...
    register('--template-directory', type=str, advanced=True, default=None,
             help='Exclude these targets from dependency resolution during workspace generation')
  ...
  def __init__(self, *args, **kwargs):
    ...
    self._renderer = MustacheRenderer(self.get_options().template_directory, __name__)
  ...
  def generate_project(self, project):
    ...
    with safe_open(self.project_filename, 'w') as output:
      output.write(self._renderer.render_name(
        _TEMPLATE, {'project': configured_project}))
